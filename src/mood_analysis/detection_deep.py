...
def face_detection(cv2):
  ...
  while True:
    ...
    frame = face_recognition.load_image_file('src/mood_analysis/Images/Selva.png')
    ...
    for (x, y, w, h) in faces:
        ...
        result = DeepFace.analyze(img_path = "src/mood_analysis/Images/Selva.png", actions=['age', 'gender', 'race', 'emotion'], enforce_detection=False)
    ...
    for (top, right, bottom, left), name in zip(face_locations, face_names):
        ...
        cv2.imwrite(os.path.join('src/mood_analysis/Images/convert.png'), frame) 
        with open("src/mood_analysis/Images/convert.png", "rb") as f:
...
def post_results(name,age,gender,emotion,encoded_image):
    ...
    r = requests.post(url='https://okotech.ai/api/receive_data', json=json_to_export)
